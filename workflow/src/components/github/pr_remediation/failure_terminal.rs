...
use crate::components::github::pr_followup_types::PrFollowupBinding;
...
fn resolve_retry_state_locked(
    ...
    let plan = match store.read_recoverable_current_json(binding, "pr-remediation-plan")? {
        crate::components::github::pr_followup_artifacts::RecoverableCurrentArtifact::Missing => {
            None
        }
        crate::components::github::pr_followup_artifacts::RecoverableCurrentArtifact::Valid(
            value,
        ) => Some(value),
        crate::components::github::pr_followup_artifacts::RecoverableCurrentArtifact::Corrupt => {
