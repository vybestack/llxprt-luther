...
/// Sanitizes a string for safe use as a single path segment. Replaces any
/// character that is not alphanumeric, `-`, `_`, or `.` with `_`. This prevents
/// path traversal (`..`, `/`, ``) from entering path components constructed
/// from user-influenced identifiers like `artifact_family` or `producer_step_id`.
