...
const GRAPHQL_REVIEW_THREADS_QUERY: &str = r#"
...
fn review_thread_page_argv(binding: &PrFollowupBinding, cursor: Option<&str>) -> Vec<String> {
    let mut argv = vec![
        "gh".to_string(),
        "api".to_string(),
        "graphql".to_string(),
...
fn review_thread_page_value(value: &Value) -> Value {
    value
...
fn review_thread_page_nodes(thread_value: &Value) -> Vec<Value> {
    thread_value
        .get("nodes")
...
fn review_thread_page_has_next(thread_value: &Value, page_count: u64) -> bool {
...
fn review_thread_page_cursor(thread_value: &Value) -> Option<String> {
    thread_value
    ...
    loop {
        ...
        if page_len < 100 || page >= 20 {
            if page >= 20 && page_len == 100 {
                eprintln!("warning: pagination limit reached for check-runs; results may be truncated");
            }
            break;
        }
        page += 1;
...
fn readiness_signal_head_sha(check: &Value, binding: &PrFollowupBinding) -> String {
    check
        ...
        .and_then(Value::as_str)
        .unwrap_or_else(|| {
            let name = check.get("name").and_then(Value::as_str).unwrap_or_default();
...
fn resolve_remote_marker_group(
    identity: &str,
    markers: &[RemoteFeedbackMarker],
    observation: &mut FeedbackObservation,
...
fn has_conflicting_remote_marker_duplicates(
...
fn remote_marker_conflict_json(identity: &str, markers: &[RemoteFeedbackMarker]) -> Value {
    json!({
...
fn audit_current_head_remote_marker(
    identity: &str,
...
fn ignore_feedback_completed_remotely(
    binding: &PrFollowupBinding,
    observation: &mut FeedbackObservation,
) {
...
fn current_head_completed_remote_keys(
    binding: &PrFollowupBinding,
...
fn retain_uncompleted_feedback_items(
    binding: &PrFollowupBinding,
    observation: &mut FeedbackObservation,
...
fn retain_uncompleted_stale_feedback_items(
    binding: &PrFollowupBinding,
    observation: &mut FeedbackObservation,
    completed_keys: &BTreeSet<(String, String)>,
) {
...
fn pending_marker_actions_from_artifact(
...
fn write_marker_validation_failure(
    store: &PrFollowupArtifactStore,
    binding: &PrFollowupBinding,
    step_id: &str,
    step_order: u64,
...
struct RemoteMarkerScan {
    completed: BTreeSet<String>,
    malformed: Vec<Value>,
}

fn scan_remote_marker_completions(
    runner: &dyn GithubPrCommandRunner,
    binding: &PrFollowupBinding,
...
fn record_completed_remote_marker(
    binding: &PrFollowupBinding,
    marker: &RemoteFeedbackMarker,
...
fn process_pending_marker_actions(
...
fn marker_report_state(report: &Value) -> &'static str {
...
fn empty_pending_marker_artifact() -> Value {
    json!({
        "pending_actions": [],
...
fn refresh_pending_marker_actions_from_current_artifacts(
    ...
    let Some(evaluations) = read_optional_current_json(store, binding, "feedback-evaluations") else {
