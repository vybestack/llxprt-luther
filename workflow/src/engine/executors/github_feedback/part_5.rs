...
fn fallback_binding(context: &StepContext, params: &Value) -> Result<PrFollowupBinding, EngineError> {
    Ok(PrFollowupBinding {
        ...
        pr_number: {
            ...
            raw.parse().map_err(|err| {
                github_feedback_error(format!("invalid pr_number '{raw}': {err}"))
            })?
        },
...
fn require_binding_identity(context: &StepContext, params: &Value) -> Result<PrFollowupBinding, EngineError> {
    Ok(PrFollowupBinding {
        schema_version: PR_FOLLOWUP_SCHEMA_VERSION,
        run_id: context.run_id().to_string(),
