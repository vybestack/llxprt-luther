...
pub struct StepContext {
    ...
    daemon_managed: bool,
    /// Immutable workspace dev/inode authorization captured by the
    /// `workspace_ownership_verify` step. Stored outside mutable workflow
    /// variables so a subsequent step (e.g. `shell`) can require the exact
    /// authorized identity without trusting a value that could be overwritten
    /// by workflow variables. `None` until the verify step populates it.
    workspace_authorization: Option<crate::engine::workspace_ownership::WorkspaceAuthorization>,
...
impl StepContext {
    ...
    /// The immutable workspace dev/inode authorization captured by the
    /// `workspace_ownership_verify` step, if any. Returns `None` until the
    /// verify step populates it. The shell step requires any workspace
    /// descriptor it opens to match this identity exactly.
    #[must_use]
    pub fn workspace_authorization(
        &self,
    ) -> Option<&crate::engine::workspace_ownership::WorkspaceAuthorization> {
        self.workspace_authorization.as_ref()
    }

    /// Set the immutable workspace dev/inode authorization captured by the
    /// `workspace_ownership_verify` step. This is the canonical setter for the
    /// authorization: it is stored outside mutable workflow variables so it
    /// cannot be overwritten by a subsequent step's variables. Once set, the
    /// authorization is immutable for the lifetime of the context: a later
    /// call is a no-op rather than silently replacing a verified identity.
    pub fn set_workspace_authorization(
        &mut self,
        authorization: crate::engine::workspace_ownership::WorkspaceAuthorization,
    ) {
        if self.workspace_authorization.is_none() {
            self.workspace_authorization = Some(authorization);
        }
    }
...
/// Re-export of [`crate::engine::shell_safe_tokens::validate_shell_safe_tokens`]
/// for callers that import from this module.
///
/// @plan:PLAN-20260722-ISSUE158-SHELL-SAFE-TOKENS
pub use crate::engine::shell_safe_tokens::validate_shell_safe_tokens;
...
impl ExecutorRegistry {
    ...
    fn register_core_executors(&mut self) {
        ...
        self.register(
            "workspace_ownership",
            Box::new(crate::engine::executors::WorkspaceOwnershipExecutor),
        );
        self.register(
            "workspace_ownership_verify",
            Box::new(crate::engine::executors::WorkspaceOwnershipVerifyExecutor),
        );
        self.register(
            "git_config_publish",
            Box::new(crate::engine::executors::GitConfigPublishExecutor),
        );
    }
