...
use crate::persistence::{
    ...
    RunMetadata, RunStatus, StateSnapshot, CHECKPOINT_STATUS_WAITING,
};
...
impl EngineRunner {
    ...
    fn persist_registry_step_state(
        &self,
        conn: &Connection,
        current_step_id: &str,
        outcome: &StepOutcome,
    ...
    fn protect_failure_cleanup_lease(
        &self,
        conn: &Connection,
    ...
    fn is_safe_failure_category(category: &str) -> bool {
        !category.is_empty()
            && category.len() <= 32
    ...
    fn build_failure_cleanup_state(
        &self,
        failed_step: &str,
        next_step: &str,
        outcome: &StepOutcome,
    ...
    fn is_failure_cleanup_step(&self, step_id: &str) -> bool {
