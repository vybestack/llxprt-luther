...
fn test_resolve_workflow_type_from_toml() {
    ...
    assert_eq!(
        ...
        11,
        "Expected 11 steps in issue-fix-v1"
    );
    assert_eq!(
        workflow_type.transitions.len(),
        12,
...
fn test_malformed_workflow_type_returns_validation_error() {
    ...
    let _malformed_toml = r#"
workflow_type_id = "malformed"
# Missing required [steps] and [transitions]
"#;

    // Note: This test would use parse_workflow_type_toml if we had a contract,
...
fn test_workflow_type_validation_checks_required_fields() {
    ...
    // The validation logic is covered by Phase 05
}
