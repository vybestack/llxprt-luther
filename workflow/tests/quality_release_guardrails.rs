...
fn test_xtask_qa_exists() {
    ...
    assert!(xtask_dir.is_dir(), "xtask directory should exist at {:?}", xtask_dir);
    ...
    match output {
        Ok(output) => {
            ...
            assert!(
                ...
                "qa command should exist in xtask. stderr: {}",
                stderr
            );
        }
        Err(e) => {
            ...
            assert!(
                ...
                "Expected cargo or path error, got: {}",
                e
            );
        }
    }
...
fn test_pr_quality_workflow_exists() {
    ...
    let workspace_root = workspace_root();
    let workflow_path = workspace_root.join(".github").join("workflows").join("pr-quality.yml");
    
    // WHEN: examining the workflow file
    // THEN: file should exist
    assert!(
        workflow_path.is_file(),
        "PR quality workflow should exist at {:?}",
        workflow_path
    );
    ...
    assert!(
        ...
        "Workflow should have format, lint, or test jobs. Content: {}",
        content
    );
...
fn test_release_workflow_exists() {
    // GIVEN: the repository should have GitHub Actions workflows
    let workspace_root = workspace_root();
    let workflow_path = workspace_root.join(".github").join("workflows").join("release.yml");
    ...
    assert!(
        has_release,
        "Workflow should be related to release. Content: {}",
        content
    );
    
    // Should build the release binary
    assert!(
        content.contains("cargo build --release") || content.contains("cargo build") ||
        content.contains("release"),
...
fn test_release_workflow_triggers_on_tag() {
    ...
    assert!(
        ...
        "Release workflow should trigger on tags or have manual dispatch. Content: {}",
        content
    );
...
fn test_release_secrets_validation() {
    ...
    assert!(
        has_token_ref,
        "Release workflow should reference required secrets. Content: {}",
        content
    );
