...
fn test_toml_json_produce_equivalent_workflow_type() {
    ...
    let toml_content = fs::read_to_string(&toml_path)
        .expect("Failed to read TOML fixture");
    ...
    assert!(toml_result.is_ok(), "TOML parsing should succeed: {:?}", toml_result.err());
    assert!(json_result.is_ok(), "JSON parsing should succeed: {:?}", json_result.err());
    ...
    assert_eq!(
        toml_workflow.workflow_type_id,
        json_workflow.workflow_type_id,
    ...
    for (i, (toml_step, json_step)) in toml_workflow.steps.iter()
        ...
        .enumerate()
    {
        ...
        assert_eq!(
            ...
            "step_type mismatch for step {}", toml_step.step_id
        );
        assert_eq!(
            toml_step.description, json_step.description,
            "description mismatch for step {}", toml_step.step_id
        );
    }
    ...
    for (i, (toml_trans, json_trans)) in toml_workflow.transitions.iter()
    ...
    // Verify guard config
    assert_eq!(
        toml_workflow.guards.max_retries,
        json_workflow.guards.max_retries,
        "max_retries should match"
    );
    assert_eq!(
        toml_workflow.guards.timeout_seconds,
        json_workflow.guards.timeout_seconds,
        "timeout_seconds should match"
    );
    assert_eq!(
        toml_workflow.guards.require_approval,
        json_workflow.guards.require_approval,
...
fn test_toml_json_produce_equivalent_config() {
    ...
    assert_eq!(
        toml_config.config_id,
        json_config.config_id,
        "config_id should match"
    );
    assert_eq!(
        toml_config.workflow_type_id,
        json_config.workflow_type_id,
        "workflow_type_id should match"
    );
    
    // Verify runtime config
    assert_eq!(
        toml_config.runtime.timeout_seconds,
        json_config.runtime.timeout_seconds,
        "runtime.timeout_seconds should match"
    );
    assert_eq!(
        toml_config.runtime.max_retries,
        json_config.runtime.max_retries,
        "runtime.max_retries should match"
    );
    assert_eq!(
        toml_config.runtime.parallel_steps,
        json_config.runtime.parallel_steps,
        "runtime.parallel_steps should match"
    );
    assert_eq!(
        toml_config.runtime.log_level,
        json_config.runtime.log_level,
    ...
    // Verify guard limits
    assert_eq!(
        toml_config.guard_limits.max_iterations,
        json_config.guard_limits.max_iterations,
        "guard_limits.max_iterations should match"
    );
    assert_eq!(
        toml_config.guard_limits.max_file_changes,
        json_config.guard_limits.max_file_changes,
        "guard_limits.max_file_changes should match"
    );
    assert_eq!(
        toml_config.guard_limits.max_tokens,
        json_config.guard_limits.max_tokens,
        "guard_limits.max_tokens should match"
    );
    assert_eq!(
        toml_config.guard_limits.max_cost,
        json_config.guard_limits.max_cost,
...
fn test_workflow_fixtures_have_both_formats() {
    ...
    let entries = fs::read_dir(&valid_dir)
        .expect("Failed to read valid workflows directory");
    ...
    for toml_name in &toml_files {
        assert!(
            ...
            "Missing JSON fixture for {}.toml", toml_name
        );
    }
    ...
    for json_name in &json_files {
        assert!(
            ...
            "Missing TOML fixture for {}.json", json_name
        );
    }
}
...
fn test_config_fixtures_have_both_formats() {
    ...
    // WHEN: scanning the valid directory
    let entries = fs::read_dir(&valid_dir)
        .expect("Failed to read valid workflow-configs directory");
...
fn test_json_parsing_handles_optional_fields() {
    ...
    assert!(result.is_ok(), "Minimal JSON should parse: {:?}", result.err());
...
"#;
    ...
    // THEN: parsing succeeds with optional fields as None
    assert!(result.is_ok(), "Minimal TOML should parse: {:?}", result.err());
