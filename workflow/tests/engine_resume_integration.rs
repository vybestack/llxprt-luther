...
use luther_workflow::persistence::{
    Checkpoint, StateSnapshot, save_checkpoint, load_checkpoint,
};
...
fn test_resume_from_checkpoint_continues_at_step() {
    ...
    let resumed_instance = WorkflowInstance::create_with_run_id(
        workflow_type, 
        config, 
        &run_id
    );
    ...
    if resumed {
        assert_eq!(resumed_runner.current_step(), "step_b",
    ...
    match loaded_checkpoint {
        Ok(Some(cp)) => {
            assert_eq!(cp.run_id, run_id, "Checkpoint should be for the correct run");
            assert_eq!(cp.step_id, "step_b", "Should resume from step_b");
            assert_eq!(cp.state_snapshot.status, "interrupted", "Status should be interrupted");
        }
        Ok(None) => {
...
fn test_interrupt_persists_resumable_checkpoint() {
    ...
    match interrupt_result {
        Ok(RunOutcome::Interrupted { step_id }) => {
            ...
            assert!(!step_id.is_empty(), "Interrupted outcome should include step_id");
...
fn test_resume_preserves_loop_and_retry_counters() {
    ...
    match loaded {
        Ok(Some(cp)) => {
            assert_eq!(cp.state_snapshot.loop_count, 2,
                "Loop count should be preserved at 2");
            assert_eq!(cp.state_snapshot.retry_count, 1,
