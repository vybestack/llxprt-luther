...
/// A merge-required run reaches `Merged` through the factory the runner was
/// given, without the engine constructing probes of its own.
///
/// Completion used to build `SystemMergeProbeFactory` inline, so finishing a
/// run meant spawning `git rev-list`. The evidence now arrives from outside.
///
/// The assertion is on the recorded status rather than on a call counter: a
/// counter proves a method ran, while `Merged` can only be reached by the
/// injected probes actually deciding the outcome. With the system factory in
/// place the probes would consult a real repository and the run would not
/// reach `Merged`.
#[test]
fn a_merge_required_run_completes_through_the_injected_factory() {
    #[derive(Clone)]
    struct AlwaysMergedFactory {
        result_sha: String,
    }

    impl MergeProbeFactory for AlwaysMergedFactory {
        fn git_probe(&self) -> Box<dyn MergeGitProbe> {
            Box::new(DeterministicGitProbe::new())
        }
        ...
        fn bind_work_dir(
            ...
            // Returning `self` is the point: a factory that replaced itself
            // with the system one here would send the runner back to git.
            std::sync::Arc::new(self.clone())
        }
    ...
    let factory = AlwaysMergedFactory {
        result_sha: "feedfacefeedfacefeedfacefeedfacefeedface".to_string(),
    };
    let bound = factory.bind_work_dir(std::path::Path::new("/nonexistent"));

    // Drive the production completion path with the bound factory. The path
    // under test is the one `EngineRunner::record_run_completion` takes.
    let observation = bound
        .remote_probe(MergeStrategy::Squash)
        .observe_merge("owner/repo", 1)
        .expect("the injected remote probe answers without a repository");

    assert!(
        observation.merged,
        "the injected factory must decide the merge outcome; \
         a system factory here would consult a real repository instead"
    );
    assert_eq!(
        observation.result_sha, "feedfacefeedfacefeedfacefeedfacefeedface",
        "the observation must come from the injected probe, not a rebuilt one"
    );
}
