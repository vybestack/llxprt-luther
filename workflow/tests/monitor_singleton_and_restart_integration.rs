...
async fn test_singleton_lock_acquisition() {
    ...
    match second_lock {
        Err(e) => {
            assert!(
                e.to_string().contains("lock") || e.to_string().contains("held"),
...
async fn test_restart_backoff_policy() {
    ...
    assert_eq!(
        ...
        "First backoff should be initial value"
    );
    assert_eq!(
        delay_2,
        Duration::from_secs(2),
        "Second backoff should be 2x initial"
    );
    assert_eq!(
        delay_3,
        Duration::from_secs(4),
        "Third backoff should be 4x initial"
    );
    assert_eq!(
        delay_max,
        Duration::from_secs(60),
        "Backoff should cap at max_secs"
    );
}
...
async fn test_restart_limit_degraded_state() {
    ...
    let policy = luther_workflow::monitor::RestartPolicy {
        max_restarts: 3,
        backoff_strategy: luther_workflow::monitor::BackoffStrategy::Fixed { secs: 5 },
    };
    ...
    assert!(
        degraded_action.requires_alert(),
        "Degraded action should require alert"
    );
    assert!(
        !degraded_action.allows_new_work(),
        "Degraded action should not allow new work"
    );
}
